...
<head>
    <meta charset="UTF-8">
    <title>Laporan Penggunaan Aktivitas Produk</title>
    <style>
        body {
...
<body>
    ...
    <div class="judul">
        LAPORAN PENGGUNAAN AKTIVITAS PRODUK
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right">{{ $usage->quantity_consumed }} {{ $usage->costDriver->unit ?? '' }}</td>
                <td class="text-center">{{ \Carbon\Carbon::parse($usage->usage_date)->format('d F Y') }}</td>
                <td>{{ $usage->notes ?? '-' }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="7" class="text-center">Tidak ada data penggunaan aktivitas produk.</td>
            </tr>
            @endforelse
        </tbody>
    </table>
