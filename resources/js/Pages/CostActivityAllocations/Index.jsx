...
export default function CostActivityAllocationIndex({
    ...
    // State untuk aktivitas yang difilter berdasarkan cost dan tanggal
    const [availableActivities, setAvailableActivities] = useState([]);
    const [isLoadingActivities, setIsLoadingActivities] = useState(false);
    const [driverInfo, setDriverInfo] = useState("");

    // Fungsi untuk fetch aktivitas berdasarkan cost dan tanggal
    const fetchActivitiesByCostAndDate = useCallback(
        async (costId, date) => {
            if (!costId || !date) {
            ...
            try {
                const response = await axios.post(
                    route("cost-activity-allocations.activities-by-cost"),
                    {
                        cost_id: costId,
                        date: date,
                    }
                ...
                setDriverInfo(response.data.driver_name || "");

                // Reset activity selection jika aktivitas yang dipilih tidak tersedia
                if (
                    data.activity_id &&
                    !response.data.activities.find(
                        (a) => a.id == data.activity_id
                    )
                ) {
                ...
                console.error("Error fetching activities by cost:", error);
                setAvailableActivities([]);

                if (error.response?.data?.message) {
                    setDriverInfo(error.response.data.message);
                } else {
                    setDriverInfo("");
                }
        ...
        [data.activity_id, setData]
    );

    // Effect untuk fetch aktivitas ketika cost atau tanggal berubah
    useEffect(() => {
        fetchActivitiesByCostAndDate(data.cost_id, data.allocation_date);
    }, [data.cost_id, data.allocation_date, fetchActivitiesByCostAndDate]);

    // Fungsi calculate dengan useCallback
    const calculateAllocation = useCallback(
        async (costId, activityId, allocationDate) => {
            ...
            try {
                ...
                console.log(
                    "Route URL:",
                    route("cost-activity-allocations.calculate")
                );
    ...
    const [editAvailableActivities, setEditAvailableActivities] = useState([]);
    const [isLoadingEditActivities, setIsLoadingEditActivities] =
        useState(false);
    const [editDriverInfo, setEditDriverInfo] = useState("");

    // Fungsi untuk fetch aktivitas edit berdasarkan cost dan tanggal
    const fetchEditActivitiesByCostAndDate = useCallback(
        async (costId, date) => {
            if (!costId || !date) {
            ...
            try {
                ...
                setEditDriverInfo(response.data.driver_name || "");

                if (
                    editData.activity_id &&
                    !response.data.activities.find(
                        (a) => a.id == editData.activity_id
                    )
                ) {
                    setEditData((prev) => ({ ...prev, activity_id: "" }));
                }
            } catch (error) {
                console.error("Error fetching edit activities by cost:", error);
                setEditAvailableActivities([]);

                if (error.response?.data?.message) {
                    setEditDriverInfo(error.response.data.message);
                } else {
                    setEditDriverInfo("");
                }
            } finally {
        ...
        [editData.activity_id, setEditData]
    );

    // Effect untuk fetch aktivitas edit ketika cost atau tanggal berubah
    useEffect(() => {
        if (isEditDialogOpen && editData.cost_id && editData.allocation_date) {
            fetchEditActivitiesByCostAndDate(
                editData.cost_id,
                            ...
                            <div className="relative">
                                <CardHeader className="pb-6 bg-purple-700 border-b border-purple-500">
                                    ...
                                    <p className="text-purple-100 text-sm mt-2 font-medium">
                                        Masukkan informasi alokasi biaya ke
                                        aktivitas.
                                    </p>
                                </CardHeader>
                                ...
                                <CardContent className="pt-6 px-6 pb-6">
                                    ...
                                    >
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            ...
                                            {/* Input Aktivitas (Select) - Dinamis berdasarkan cost dan tanggal */}
                                            <div className="space-y-3">
                                                <Label
                                                    htmlFor="activity_id"
                                                    className="text-sm font-semibold text-white flex items-center gap-2"
                                                                ...
                                                                placeholder={
                                                                    !data.cost_id
                                                                        ...
                                                                        : availableActivities.length ===
                                                                          0
                                                                        ? "Tidak ada aktivitas dengan driver yang sama"
                                                                        : "Pilih Aktivitas"
                                                                }
                                                            />
                                                        </SelectTrigger>
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center gap-2 w-full md:w-auto">
                                        ...
                                        <Select
                                            ...
                                            onValueChange={(value) => {
                                                ...
                                                router.get(
                                                    route(
                                                        "cost-activity-allocations.index"
                                                    ),
                                ...
                                {allocations.data.length === 0 ? (
                                    <div className="text-center py-16">
                                        ...
                                        <p className="text-gray-500 text-lg">
                                            Mulai tambahkan alokasi biaya ke
                                            aktivitas.
                                        </p>
                                    </div>
                                ) : (
                ...
                <DialogContent className="sm:max-w-2xl">
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <Label htmlFor="editAllocationDate">
                                    Tanggal Alokasi
                                </Label>
                                <Input
                            ...
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) =>
                                        setEditData("cost_id", parseInt(value))
                                    }
                                >
                            ...
                            {/* Edit Aktivitas - Dinamis berdasarkan cost dan tanggal */}
                            <div>
                                        ...
                                        <SelectValue
                                            placeholder={
                                                !editData.cost_id
                                                    ...
                                                    : isLoadingEditActivities
                                                    ? "Memuat aktivitas..."
                                                    : editAvailableActivities.length ===
                                                      0
                                                    ? "Tidak ada aktivitas dengan driver yang sama"
                                                    : "Pilih Aktivitas"
                                            }
                            ...
                            <div>
                                <Label htmlFor="editAllocatedAmount">
                                    Jumlah Dialokasikan
                                </Label>
                                <Input
